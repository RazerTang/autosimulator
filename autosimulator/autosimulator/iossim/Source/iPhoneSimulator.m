...
- (int)launchApp:(NSString *)path withFamily:(NSString *)family
  ...
  if (stderrPath) {
    ...
    [self createStdioFIFO:&stderrFileHandle ofType:@"stderr" atPath:&stderrPath];
            [config setStderrFileHandle:stderrFileHandle];
  ...
  if (stdoutPath) {
    ...
    [self createStdioFIFO:&stdoutFileHandle ofType:@"stdout" atPath:&stdoutPath];
      [config setStdoutFileHandle:stdoutFileHandle];
  }
...
//    
//    fcntl(STDERR_FILENO, F_GETPATH, &paths);
//    if (verbose) {
//        nsprintf(@"path %@",[NSString stringWithUTF8String:paths]);
//    }
//    [config setSimulatedApplicationStdOutPath:[NSString stringWithUTF8String:paths]];
//    [config setSimulatedApplicationStdErrPath:[NSString stringWithUTF8String:paths]];
...
- (void)session:(DTiPhoneSimulatorSession *)session didEndWithError:(NSError *)error {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"KRunNextSimulator" object:nil];

}


-(void)stop{
    [session stopLocationSimulation];
    [session requestEndWithTimeout:30];
